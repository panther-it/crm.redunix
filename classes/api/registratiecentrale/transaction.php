...
// URL Interface
class RegistratieCentraleTransaction {

	// When filled in, the auth,user and pass values are automatically supplied in each request
	var $authtype = "plain";    // Tells server if password is sent in 'plain' or 'md5', when useSSL = false only 'md5' is allowed
	var $user     = "redunix";
	var $pass     = "oosha4Ah";

	var $host   = "secure.registratiecentrale.nl";
	var $url    = "/api/?";
	var $useSSL = true;
	...
	function DoTransaction() {
		...
		if ( !$socket ) {
			function strerror()
				{
				return "Could not connect to Server -Please try again Later ($errno, $errstr)";
				}
